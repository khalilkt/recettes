...
function filterArchives() {
    ...
    etat = $("#etat").val();
    type_ar = $("#type_ar").val();
    $('#datatableshow').DataTable().ajax.url(racine + 'archives/getDT/' + categ +'/'+ structure +'/'+ date_min +'/'+ date_max+'/'+ etat+'/'+ type_ar).load();
}


    

    function changeType_Ar(){
        var categ = $('#type').val();
        // alert(type);
        if (categ == 1){
            $( '#type_arch' ).show();
    ...
    function getPage(type) {
        $.ajax({
            type: 'get',
            url: racine + 'archives/getPage/'+type,
            success: function (data) {
                $("#page").html("data");
                // $("#"+nameDiv).show();
                resetInit();
    ...
    date_min = $("#date_debut").val();
    date_max = $("#date_fin").val();
    $('#datatableshow').DataTable().ajax.url(racine + 'courriers/getDT/' + type +'/'+ service +'/'+ origine +'/'+ niveau +'/'+ date_min +'/'+ date_max).load();
}

function changeType_courrier(){
        var type = $('#type').val();
        // alert(type);
        if (type == 1){
            $( '#orig' ).html("Origine");
    ...
    //export liste des Courriers en excel 
    function getObjectPDF(lien)
    {
        document.formst.action = racine + lien;
