...
function annulerRole(id) {
    var confirme = confirm("Êtes-vous sûr de vouloir annuler les enregistrements de ce role");
    if(confirme){
        $.ajax({
            type: 'get',
...
function suiviprogrammes() {
    fichier = $("#fichier").val();
    if (fichier !=''){
    var confirme = confirm("Êtes-vous sûr de vouloir importer cet enregistrement");
    if(confirme) {
...
function excelEquipement1(){
    ...
    document.formst.action = 'equipements/exportExcel/'+type+'/'+ secteur+'/'+ localite+'';
...
function imprimerFicheCommunal(){
    ...
    document.formcom.action = 'equipements/exportPDFFicheCommune/'+equipement+'/'+ employe+'/'+ budget+'/'+ contribuables+'/'+ archive+'/'+ actes+'/'+ localites+'/'+ annee+'';
...
function pdfBudget(id,etat){
    ...
    classe=0;
    if (etat==1){
        document.formbdg.action = 'finances/exportPDFBudgetFiltre/'+id+'/'+ niveau_affichage+'/'+ classe+'';
        document.formbdg.target = "_blank";    // Open in a new window
        document.formbdg.submit();             // Submit the page
    }
    if (etat==2) {
        document.formbdg1.action = 'finances/exportPDFBudgetFiltre/'+id+'/'+ niveau_affichage+'/'+ classe+'';
    ...
    if (etat==3) {
        ...
        document.formbdg2.action = 'finances/exportPDFBudgetFiltre/'+id+'/'+ niveau_affichage+'/'+ classe+'';
...
function excelBudget(id,etat){
    ...
    classe=0;
    if (etat==1){
        document.formbdg.action = 'finances/exportEXCELBudgetFiltre/'+id+'/'+ niveau_affichage+'/'+ classe+'';
        document.formbdg.target = "_blank";    // Open in a new window
        document.formbdg.submit();             // Submit the page
    }
    if (etat==2) {
        document.formbdg1.action = 'finances/exportEXCELBudgetFiltre/'+id+'/'+ niveau_affichage+'/'+ classe+'';
        document.formbdg1.target = "_blank";    // Open in a new window
        document.formbdg1.submit();             // Submit the page
    }
    if (etat==3) {
        ...
        document.formbdg2.action = 'finances/exportEXCELBudgetFiltre/'+id+'/'+ niveau_affichage+'/'+ classe+'';
        document.formbdg2.target = "_blank";    // Open in a new window
        document.formbdg2.submit();             // Submit the page
    }
    return true;
...
function exporterSuiviExecution(element){
    ...
    document.formst.action = 'finances/exporterSuiviExecution/'+id+'/'+date1+'/'+date2+'/'+sence+'/'+texte+'/'+detail+'/'+niveau+'';
    document.formst.target = "_blank";    // Open in a new window
    document.formst.submit();             // Submit the page
    return true;
}
function exporterSuiviExecutionExcel(element){
    ...
    document.formst.action = 'finances/exporterSuiviExecutionExcel/'+id+'/'+date1+'/'+date2+'/'+sence+'/'+texte+'/'+detail+'/'+niveau+'';
    document.formst.target = "_blank";    // Open in a new window
    document.formst.submit();             // Submit the page
    return true;
}
...
function exporterSituationBudgetaire(element){
    ...
    document.formst.action = 'finances/exporterSituationBudgetaire/'+id+'/'+date1+'/'+date2+'/'+type+'/'+detail+'';
    document.formst.target = "_blank";    // Open in a new window
    document.formst.submit();             // Submit the page
    return true;
}
...
function exporterSituationBudgetaireExcel(element){
    ...
    document.formst.action = 'finances/exporterSituationBudgetaireExcel/'+id+'/'+date1+'/'+date2+'/'+type+'/'+detail+'';
    document.formst.target = "_blank";    // Open in a new window
    document.formst.submit();             // Submit the page
    return true;
}
...
function pdfEquipement(){
    ...
    document.formst.action = 'equipements/exportPDF/' + type +'/'+ secteur+'/'+ localite+'';
    document.formst.target = "_blank";    // Open in a new window
    document.formst.submit();             // Submit the page
    return true;
...
function filterTypePayement() {
    ...
    $('#datatableshow').DataTable().ajax.url(racine + 'contribuables/getDT/' + type).load();
}
...
function filterSecteurEquipement() {
    ...
    $('#datatableshow').DataTable().ajax.url(racine + 'equipements/getDTE/' + type +'/'+ secteur+'/'+ localite).load();
}
...
function filtrebudgets(id){
    ...
    $.ajax({
        type: 'get',
        url: racine + 'finances/BudgetFiltre/'+id+'/'+ niveau_affichage+'/'+ classe,
        success: function (data) {
            $("#presentation").html('');
...
function filterActeModele() {
    ...
    $('#datatableshow').DataTable().ajax.url(racine + 'actes/getDT/' + modele+'/'+ date1+'/'+ date2 ).load();
}
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data !=''){
                    ...
                    $('select[name="ordres"]').append('<option  value="0">'+premiere+'</option>');
                    $.each(data, function(key, value) {
                        $('select[name="ordres"]').append('<option value="'+ value.ordre +'">'+apres+' "'+value.content_value+'"</option>');
                    });
                    $('select[name="ordres"]').append('<option  value="D" selected>'+dernier+'</option>');
                }
                else{
                    $('select[name="ordres"]').append('<option  value="P">'+premiere+'</option>');
                }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data !=''){
                    ...
                    $('#ordresedit').append('<option  value="0">'+premiere+'</option>');
                    $.each(data, function(key, value) {
                        $('#ordresedit').append('<option value="'+ value.ordre +'">'+apres+' "'+value.content_value+'"</option>');
                    });
                    $('#ordresedit').append('<option  value="D" selected>'+dernier+'</option>');
                }
                else{
...
function parentLigne(id,premiere,dernier,apres,suite_elemet,pas_de_parent){
    ligne = $("#ligne").val();
    ...
    if (ligne !='' || ligne !=0){
        $.ajax({
            ...
            success: function (data) {
                $('#alignement').show();
                $('#ordres').show();
                if (position !='<br>'){$('select[name="ordres"]').empty();}
                $('select[name="parent"]').append('<option  value="0">'+pas_de_parent+'</option>');
                if (data !=''){
                    if (position !='<br>'){
                        $('select[name="ordres"]').append('<option  value=""></option>');
                        $('select[name="ordres"]').append('<option  value="0">'+premiere+'</option>');
                ...
                $.each(data, function(key, value) {
                    $('select[name="parent"]').append('<option value="'+ value.id +'">'+suite_elemet+'  "'+value.content_value+'"</option>');
                    if (position !='<br>') {
                        $('select[name="ordres"]').append('<option value="' + value.ordre + '">'+apres+' "' + value.content_value + '"</option>');
                    }
                });
...
function parentLigneedit(id,premiere,dernier,apres,suite_elemet,pas_de_parent){
    ligne = $("#ligneedit").val();
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (position !='<br>'){
                    ...
                    $("#parentedit").append('<option  value="0">'+pas_de_parent+'</option>');
                if (data !=''){
                    if (position !='<br>'){
                        $('#ordresedit').append('<option  value=""></option>');
                        $('#ordresedit').append('<option  value="0">'+premiere+'</option>');
                ...
                $.each(data, function(key, value) {
                    $("#parentedit").append('<option value="'+ value.id +'">'+suite_elemet+'  "'+value.content_value+'"</option>');
                    if (position !='<br>') {
                        $('#ordresedit').append('<option value="' + value.ordre + '">'+apres+' "' + value.content_value + '"</option>');
                    }
                });
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data !=''){
                    ...
                    $('select[name="ligne"]').append('<option  value="0">'+premiere+'</option>');
                    $.each(data, function(key, value) {
                        $('select[name="ligne"]').append('<option value="'+ value.ligne +'">'+apres+' "'+value.content_value+'" </option>');
                    });

                    $('select[name="ligne"]').append('<option value="D" selected>'+dernier+'</option>');
                }
                else{
                    $('select[name="ligne"]').append('<option  value="P">'+premiere+'</option>');
                }
                resetInit();
            },
            error: function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data !=''){
                    $('select[name="ligne"]').append('<option  value=""></option>');
                     $.each(data, function(key, value) {
                        $('select[name="ligne"]').append('<option value="'+ value.ligne +'"> '+value.content_value+'</option>');
                    });
                }
                else{
                    $('select[name="ligne"]').append('<option  value="P">'+premiere+'</option>');
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data !=''){
                    ...
                    $("#ligneedit").append('<option  value="0">'+premiere+'</option>');
                    $.each(data, function(key, value) {
                        $("#ligneedit").append('<option value="'+ value.ligne +'">'+apres+' "'+value.content_value+'" </option>');
                    });

                    $("#ligneedit").append('<option value="D" selected>'+dernier+'</option>');
                }
                else{
                    $("#ligneedit").append('<option  value="P">'+premiere+'</option>');
                }
                resetInit();
            },
            error: function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data !=''){
                    $("#ligneedit").append('<option  value=""></option>');
                     $.each(data, function(key, value) {
                         $("#ligneedit").append('<option value="'+ value.ligne +'"> '+value.content_value+'</option>');
                    });
                }
                else{
                    $("#ligneedit").append('<option  value="P">'+premiere+'</option>');
...
function recuperemontant(){
    ...
    if (echance !='')
    {
        if (echance !='all'){
            $.ajax({
                type: 'get',
                url: racine + 'contribuables/recuperemontant1/'+protocol+'/'+echance,
                success: function (data) {
                    // alert(data)
...
function payercontibiable(annee) {
   ...
   // alert(type_payement)
    ...
    url = racine + 'contribuables/payercontibiable/'+annee+'/'+type_payement;
    $.ajax({
        type: 'get',
        url: url,
        success: function (data) {
...
function pdfSuiviPayementCtb(annee){
    ...
    document.formst.action = 'contribuables/pdfSuiviPayementCtb/' + annee+'/'+ filtrage + '/'+ date1 +'/'+ date2+'/'+role;
    document.formst.target = "_blank";    // Open in a new window
    document.formst.submit();             // Submit the page
    return true;
}
...
function sutiationcontribuablePDF(id){
    ...
    if (annee!='')
    {
        document.formspdf.action = 'contribuables/sutiationcontribuablePDF/'+id+'/'+annee;
        document.formspdf.target = "_blank";    // Open in a new window
        document.formspdf.submit();             // Submit the page
        return true;
    }
