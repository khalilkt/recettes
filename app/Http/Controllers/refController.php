...
class refController extends Controller
{
    ...
    public function get_title($ref)
    {
        $title = "";
        switch ($ref) {
            case 'RefNiveauImportance':
                $title = trans('text_archive.niveau_importance');
                $title_modif = trans('text_archive.modification_niv_impotc');
                $title_nouveau = trans('text_archive.new_niv_impoce');
                break;
            case 'ArOrigine':
                $title = trans('text_archive.origines');
                $title_modif = trans('text_archive.modification_origine');
                $title_nouveau = trans('text_archive.new_origine');
                break;
            case 'RefTypeArchive':
                $title = trans('text_archive.types_archive');
                $title_modif = trans('text_archive.modification_types_archive');
                $title_nouveau = trans('text_archive.new_type_archive');
                break;
            case 'ArStructure':
                $title = trans('text_archive.structures');
                $title_modif = trans('text_archive.modification_structure');
                $title_nouveau = trans('text_archive.new_structure');
                break;
            case 'ArEmplacement':
                $title = trans('text_archive.emplacements');
                $title_modif = trans('text_archive.modification_emplacement');
                $title_nouveau = trans('text_archive.new_emplacement');
                break;
            case 'ArQualite':
                $title = trans('text_archive.qualites');
                $title_modif = trans('text_archive.modification_qualite');
                $title_nouveau = trans('text_archive.new_qualite');
                break;
            case 'Domaine':
    ...
    public function get_msg($type, $id, $msg)
    {
        ...
        switch ($type) {
            ...
            case  'RefNiveauImportance':
                ...
                return trans('text_archive.le_niv_imptc') . ' "' . $object->libelle . '"  ' . $msg;
                break;
            case  'ArOrigine': 
                $object = $name::withTrashed()->find($id);
                return trans('text_archive.lorigine') . ' "' . $object->libelle . '"  ' . $msg;
                break;
            case  'RefTypeArchive':
                $object = $name::withTrashed()->find($id);
                return trans('text_archive.le_type_ar') . ' "' . $object->libelle . '"  ' . $msg;
                break;
            case  'ArStructure':
                $object = $name::withTrashed()->find($id);
                return trans('text_archive.la_structure') . ' "' . $object->libelle . '"  ' . $msg;
                break;
            case  'ArEmplacement':
                $object = $name::withTrashed()->find($id);
                return trans('text_archive.lemplacement') . ' "' . $object->libelle . '"  ' . $msg;
                break;
            case  'ArQualite':
                $object = $name::withTrashed()->find($id);
                return trans('text_archive.la_qualite') . ' "' . $object->libelle . '"  ' . $msg;
                break;
            case  'Domaine':
                $object = $name::withTrashed()->find($id);
    ...
    public function add_ref(RefRequest $request)
    {
        ...
        if ($param == 0) {
            ...
            if ($model == 'ArEmplacement'){
                $referentiel->type_archive = $request->type_archive;
                $code = "";
                $last_ref =  $name::where("type_archive", $request->type_archive)->orderBy('id','desc')->first();
                if($last_ref){
                    $last_code = $last_ref->code;
                    $tabs = explode('_',$last_code);
                    $order = $tabs[1] + 1;
                    $code  = $tabs[0]."_".$order;
                }
                else{
                    switch ($request->type_archive) {
                        case 1:
                            $code = "A_1";
                            break;
                        case 2:
    ...
    public function delete_ref($ref, $id)
    {
        ...
        switch ($model) {
            case 'ArEmplacement':{
                if($referentiel->archives->count()>0){
                  return redirect('/ref/'.$model)->with('errormsg',trans('text_archive.empla_cant_be_del_bcuz_of_archives'));
            ...
            case 'ArOrigine':{
                if($referentiel->archives->count()>0){
                  return redirect('/ref/'.$model)->with('errormsg',trans('text_archive.origine_cant_be_del_bcuz_of_archives'));
            ...
            case 'RefTypeArchive':{
                if($referentiel->archives->count()>0){
                  return redirect('/ref/'.$model)->with('errormsg',trans('text_archive.type_cant_be_del_bcuz_of_archives'));
            ...
            case 'ArQualite':{
                if($referentiel->archives->count()>0){
                  return redirect('/ref/'.$model)->with('errormsg',trans('text_archive.qualite_cant_be_del_bcuz_of_archives'));
            ...
            case 'RefNiveauImportance':{
                if($referentiel->archives->count()>0){
                  return redirect('/ref/'.$model)->with('errormsg',trans('text_archive.niveau_cant_be_del_bcuz_of_archives'));
    ...
    public function redirectTo($type, $id)
    {
        ...
        switch ($type) {
            ...
            case  'RefNiveauImportance':
                $object = $name::find($id);
                return redirect($ref)->with(['successmsg' => trans('text_archive.le_niv_imptc') . ' "' . $object->libelle . '" a été bien ajouté', 'selected' => $object->id]);
                break;
            case  'ArOrigine':
                $object = $name::find($id);
                return redirect($ref)->with(['successmsg' => trans('text_archive.lorigine') . ' "' . $object->libelle . '" a été bien ajouté', 'selected' => $object->id]);
                break;
            case  'RefTypeArchive':
                $object = $name::find($id);
                return redirect($ref)->with(['successmsg' => trans('text_archive.le_type_ar') . ' "' . $object->libelle . '" a été bien ajouté', 'selected' => $object->id]);
                break;
            case  'ArStructure':
                $object = $name::find($id);
                return redirect($ref)->with(['successmsg' => trans('text_archive.la_structure') . ' "' . $object->libelle . '" a été bien ajouté', 'selected' => $object->id]);
                break;
            case  'ArEmplacement':
                $object = $name::find($id);
                return redirect($ref)->with(['successmsg' => trans('text_archive.lemplacement') . ' "' . $object->libelle . '" a été bien ajouté', 'selected' => $object->id]);
                break;
            case  'ArQualite':
                $object = $name::find($id);
                return redirect($ref)->with(['successmsg' => trans('text_archive.la_qualite') . ' "' . $object->libelle . '" a été bien ajouté', 'selected' => $object->id]);
                break;
