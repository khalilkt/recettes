...
class AdminController extends Controller
{
    ...
    {
        if (Auth::attempt(['email' => $request->email, 'password' => $request->password, 'etat' => 1, 'supprimer' => 0])) {
            if (Auth::user()->hasAnyRight([1]))
                $link =  url('dashboard');
            elseif(Auth::user()->hasAnyRight([2,3,5]))
                $link =  url('fichescommunales');
            elseif(Auth::user()->hasAnyRight([6,7,8]))
                $link =  url('Compdec/formations');
            elseif(Auth::user()->hasAnyRight([9,10]))
                $link =  url('Compdec/portail/formations');
            return response()->json($link,200);
        } else {
            if (User::where(['email'=>$request->email, 'etat'=>0])->exists())
                return response()->json(['login'=>['Cette utilisateur est suspendu']],422);
            else
                return response()->json(['login'=>['Email ou mot de passe incorrect']],422);
        }
    }
}
